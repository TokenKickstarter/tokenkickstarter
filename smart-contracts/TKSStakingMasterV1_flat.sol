...
// File contracts/TKSStakingMasterV1.sol
...
/**
 * @title TKSStakingMasterV1
 * @dev Upgradeable multi-pool staking contract for TKS ecosystem
 * Supports: TKS, TKS-BNB LP, USDT, and native BNB staking
 * All pools reward TKS tokens with duration-based APY
 */
contract TKSStakingMasterV1 is 
    Initializable,
    ...
    UUPSUpgradeable 
{
    ...
    // TKS reward token
    IERC20 public tksToken;

    // Pool types
    enum PoolType { TKS, LP, USDT, BNB }

    // Pool configuration
    struct Pool {
        PoolType poolType;
        address stakingToken;      // 0x0 for native BNB
        uint256[] allowedDurations; // in seconds
        uint256 totalStaked;
        uint256 depositFee;        // basis points (100 = 1%)
        uint256 withdrawFee;       // basis points
        bool isActive;
        string name;
        string symbol;
    }

    // APY configuration per duration (basis points, 10000 = 100%)
    struct APYTier {
        uint256 duration;
        uint256 apy;
    }
